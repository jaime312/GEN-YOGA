...
$miriam$
where id = 10
   or lower(coalesce(nombre, '')) = 'miriam'
   or lower(coalesce(email, '')) like 'miriam%';

update public.profesionales
set descripcion = $silvia$
...
$silvia$
where id = 1
   or lower(coalesce(nombre, '')) = 'silvia'
   or lower(coalesce(email, '')) like 'silvia%';

update public.profesionales
set descripcion = $yanira$
TRAYECTORIA:
...
$yanira$
where id = 14
   or lower(coalesce(nombre, '')) = 'yanira'
