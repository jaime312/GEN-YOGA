-- ============================================================================
-- Migración 202609020055: Seguridad y Aislamiento Estricto para Perfil Profesor (v8.8)
-- Vinculación flexible y segura para profesores oficiales y de pruebas
-- ============================================================================
  ...
  if v_actor_role = 'admin' then
    ...
    select coalesce(array_agg(id), '{}'::int[])
     ...
     where (v_actor_email <> '' and lower(trim(coalesce(email, ''))) = v_actor_email)
        ...
        or (v_actor_nombre not in ('angel', 'ángel', 'silvia', 'yanira', 'miriam', 'isabel') and lower(trim(coalesce(nombre, ''))) not in ('angel', 'ángel', 'silvia', 'yanira', 'miriam', 'isabel') and lower(trim(coalesce(nombre, ''))) like '%' || v_actor_nombre || '%');
