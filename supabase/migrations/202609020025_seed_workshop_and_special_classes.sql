-- ============================================================================
-- Migration 202609020025: Seed Workshop Intro Power Vinyasa, Special Classes & Stripe Fulfillment
-- ============================================================================

begin;

-- 1. Identificar a Yanira
with teacher as (
...
taller_style as (
  ...
  where lower(btrim(coalesce(nombre, ''))) like '%taller%'
  limit 1
)
-- Asegurar Taller: Introducción a Power Vinyasa (Sábado 19 Sept 2026, 09:00 a 11:00)
...
-- Asegurar actualización de capacidad y estado activo en caso de existir ya
...
-- 3. Actualizar la función stripe_fulfill_checkout para aceptar clase_especial y taller_intro_power_vinyasa
...
begin
  ...
  if p_purchase_type = 'clase_suelta' and p_amount_total is distinct from 1500 then
  ...
  elsif p_purchase_type = 'taller_intro_power_vinyasa' and p_amount_total is distinct from 3500 then
    ...
    raise exception 'Invalid amount' using errcode = '22023';
  elsif p_purchase_type = 'clase_especial' then
    -- Importe variable según configuración de Stripe
    null;
  ...
  if v_pack_credits is not null then
    ...
    update public.profiles
     ...
     where id = p_user_id;
    get diagnostics v_profile_updated = row_count;
  elsif p_purchase_type in ('clase_especial', 'taller_intro_power_vinyasa') then
    update public.profiles
       set stripe_customer_id = coalesce(p_customer_id, stripe_customer_id)
     where id = p_user_id;
    get diagnostics v_profile_updated = row_count;
