...
BEGIN
  ...
  SELECT lower(coalesce(c.tipo_clase, '')), coalesce(c.capacidad_max, 0),
         c.fecha_inicio, coalesce(c.es_gratuita, false), lower(coalesce(c.nombre, ''))
    INTO v_type, v_capacity, v_starts_at, v_is_free, v_name
  ...
  SELECT lower(coalesce(rol, '')), coalesce(saldo_consultas_gratis, 0)
    INTO v_target_role, v_target_free_consultations
  ...
  IF v_is_free AND v_charge THEN
    ...
    IF NOT FOUND THEN
      -- Si ya no tiene saldo gratuito, desmarcar para intentar cobrar de saldo de consulta pagada
      v_is_free := false;
    END IF;
  END IF;
