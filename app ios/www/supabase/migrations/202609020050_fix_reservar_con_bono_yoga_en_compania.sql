-- ==============================================================================
-- Migración 202609020050: Reparación definitiva y soporte de reservas para Yoga en Compañía y Bono de Bienvenida
-- ==============================================================================

BEGIN;

-- 1. Asegurar columna saldo_yoga_compania en public.profiles con valor por defecto 1
...
-- 2. Asegurar que los alumnos dispongan de sus bonos de bienvenida y yoga en compañía
UPDATE public.profiles
   SET saldo_yoga_compania = 1
 WHERE lower(coalesce(rol, 'alumno')) NOT IN ('admin', 'profesor', 'trabajador', 'profesional')
   AND coalesce(saldo_yoga_compania, 0) = 0
   AND NOT EXISTS (
     SELECT 1
       FROM public.reservas_yoga r
      WHERE r.user_id = profiles.id
        AND (coalesce(r.saldo_gratis_descontado, false) OR r.welcome_companion_modality IS NOT NULL)
   );

-- 3. Actualizar función clasificadora de clases gratuitas/introductorias y yoga en compañía
...
BEGIN
  ...
  -- Si el nombre contiene palabras clave de sesión gratuita, introductoria, abierta o en compañía
  IF v_nom ~* 'introductor|bienvenida|abierta|gratis|prueba|madre|hija|compañ|compan|pareja|colegas|abuela|hijo' THEN
    RETURN true;
  END IF;
...
-- 4. Recrear función principal de reserva con bono soportando Yoga en Compañía
...
BEGIN
  ...
  -- 2. Si es clase de Yoga en Compañía
  ...
  -- 3. Si es sesión gratuita/introductoria Y el alumno tiene saldo de bono gratis o de compañía
  IF v_is_free THEN
  ...
  -- 5. Bonos Normales: Caso Bono Ilimitado
  ...
  -- 6. Bonos Normales: Packs de Clases o Saldo de Bonos
...
-- 5. Función sobrecargada para llamadas con 2 parámetros
CREATE OR REPLACE FUNCTION public.reservar_con_bono(
  p_clase_id bigint,
  p_user_id uuid
)
