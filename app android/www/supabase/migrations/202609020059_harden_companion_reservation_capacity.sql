...
begin
  ...
  select coalesce(c.capacidad_max, 0),
         coalesce(c.tipo_clase, ''),
         coalesce(c.activa, true),
         lower(trim(coalesce(c.companion_modality, '')))
  ...
  if v_class_modality <> v_modalidad_norm then
  ...
  select coalesce(sum(
    case
      when jsonb_typeof(coalesce(r.acompanantes, '[]'::jsonb)) = 'array'
        ...
        then greatest(
          coalesce(r.num_plazas_reservadas, 1),
          coalesce(r.num_plazas, 1),
  ...
  insert into public.reservas_yoga (
    ...
    v_modalidad_norm, false, false,
    v_plazas_requeridas, v_acompanantes,
  ...
  return jsonb_build_object(
    ...
    'saldo_restante', case when v_actor_role = 'admin'
      then v_saldo_compania
      else greatest(0, v_saldo_compania - 1)
    end
  );
end;
$function$;
