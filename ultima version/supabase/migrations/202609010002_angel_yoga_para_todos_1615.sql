...
-- 2. Actualizar clases existentes de las 16:00 a 17:00 en Lunes y Miércoles de Ángel a 16:15 - 17:30
with teacher as (
...
where c.profesor_id = t.profesor_id
  and extract(isodow from c.fecha_inicio at time zone 'Europe/Madrid')::integer in (1, 3)
  and (c.fecha_inicio at time zone 'Europe/Madrid')::time >= '16:00'::time
  and (c.fecha_inicio at time zone 'Europe/Madrid')::time <= '16:30'::time
  and lower(btrim(coalesce(c.nombre, ''))) = 'yoga para todos';

-- 3. Insertar las clases que falten para Lunes y Miércoles de 16:15 a 17:30
with teacher as (
  select id as profesor_id
  from public.profesionales
  where id = 13
...
target_slots as (
  ...
  cross join style s
  where extract(isodow from sd.fecha)::integer in (1, 3) -- Lunes (1) y Miércoles (3)
)
