-- ============================================================================
-- Migración 202609020053: Seguridad estricta en asistencias y reservas para profesores
-- Garantiza que un profesor/profesional solo reciba datos de sus propias clases y alumnos
-- ============================================================================

begin;

-- 1. Actualizar función RPC con SECURITY DEFINER para filtrar estrictamente por las clases del profesor
...
begin
  ...
  -- Si es admin, puede consultar todas las clases o el filtro p_clase_ids suministrado
  if v_actor_role = 'admin' then
    v_allowed_clase_ids := p_clase_ids;
  else
    -- Si es profesor / trabajador, resolver los IDs de profesional vinculados al usuario
    select coalesce(array_agg(id), '{}'::int[])
     ...
     where (v_actor_email <> '' and lower(trim(coalesce(email, ''))) = v_actor_email)
        ...
        or (v_actor_nombre <> '' and v_actor_apellidos <> '' and lower(trim(coalesce(nombre, '') || ' ' || coalesce(apellidos, ''))) = (v_actor_nombre || ' ' || v_actor_apellidos));
